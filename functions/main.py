...
def tg_send_single_media(token: str, chat_id: int, file: Path) -> None:
    ...
    method = "sendVideo" if ext in {
        ".mp4", ".mov", ".m4v", ".webm"} else "sendPhoto"
    url = _telegram_api_url(method, token)
    with open(file, "rb") as fh:
        files = {"photo" if method ==
                 "sendPhoto" else "video": (file.name, fh)}
        requests.post(url, data={"chat_id": str(
            chat_id)}, files=files, timeout=60)
...
def handle_update(update: dict, *, token: str, cookies_file: Optional[str], timeout_s: int) -> None:
    ...
    if not url:
        tg_send_message(
            token, chat_id, "Пришли, пожалуйста, ссылку на Instagram (post/reel/story/highlight).")
        return
    ...
    try:
        ...
        if not files:
            tg_send_message(
                token, chat_id, "Не удалось найти медиа по этой ссылке.")
            return
...
def telegram_webhook(req: https_fn.Request) -> https_fn.Response:
    ...
    handle_update(update, token=token,
                  cookies_file=cookies_file, timeout_s=timeout_s)
